...
pub const TF_DEFAULT_SELECTION = @as(u32, 4294967295);
pub const TS_DEFAULT_SELECTION = @as(u32, 4294967295);
...
pub const ITfReverseConversion = extern struct {
    ...
    pub fn MethodMixin(comptime T: type) type { return struct {
        pub usingnamespace IUnknown.MethodMixin(T);
        // NOTE: method is namespaced with interface name to avoid conflicts for now
        pub fn ITfReverseConversion_DoReverseConversion(self: *const T, lpstr: ?[*:0]align(1) const u16, ppList: ?*?*ITfReverseConversionList) callconv(.Inline) HRESULT {

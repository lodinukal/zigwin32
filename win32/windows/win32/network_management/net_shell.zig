...
pub const PNS_CONTEXT_CONNECT_FN = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        pwszMachine: ?[*:0]align(1) const u16,
    ) callconv(@import("std").os.windows.WINAPI) u32,
    else => *const fn(
        pwszMachine: ?[*:0]align(1) const u16,
    ) callconv(@import("std").os.windows.WINAPI) u32,
...
pub const PNS_CONTEXT_DUMP_FN = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        pwszRouter: ?[*:0]align(1) const u16,
        ppwcArguments: [*]?PWSTR,
        dwArgCount: u32,
...
pub const PFN_CUSTOM_HELP = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        hModule: ?HANDLE,
        pwszCmdToken: ?[*:0]align(1) const u16,
...
pub const PNS_OSVERSIONCHECK = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        ...
        CIMOSVersion: ?[*:0]align(1) const u16,
        CIMOSBuildNumber: ?[*:0]align(1) const u16,
        CIMServicePackMajorVersion: ?[*:0]align(1) const u16,
        CIMServicePackMinorVersion: ?[*:0]align(1) const u16,
        uiReserved: u32,
        dwReserved: u32,
...
pub const CMD_GROUP_ENTRY = extern struct {
    pwszCmdGroupToken: ?[*:0]align(1) const u16,
    dwShortCmdHelpToken: u32,
    ulCmdGroupSize: u32,
    dwFlags: u32,
    pCmdGroup: ?*CMD_ENTRY,
...
pub extern "netsh" fn MatchToken(
    pwszUserToken: ?[*:0]align(1) const u16,
    pwszCmdToken: ?[*:0]align(1) const u16,
) callconv(@import("std").os.windows.WINAPI) BOOL;
...
test {
    ...
    if (@hasDecl(@This(), "PFN_CUSTOM_HELP")) { _ = PFN_CUSTOM_HELP; }

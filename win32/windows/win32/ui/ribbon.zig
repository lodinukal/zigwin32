...
pub const UI_INVALIDATIONS = enum(i32) {
    ...
    pub fn initFlags(o: struct {
        ...
        return @as(UI_INVALIDATIONS, @enumFromInt(
              (if (o.STATE == 1) @intFromEnum(UI_INVALIDATIONS.STATE) else 0)
            | (if (o.VALUE == 1) @intFromEnum(UI_INVALIDATIONS.VALUE) else 0)
            | (if (o.PROPERTY == 1) @intFromEnum(UI_INVALIDATIONS.PROPERTY) else 0)
            | (if (o.ALLPROPERTIES == 1) @intFromEnum(UI_INVALIDATIONS.ALLPROPERTIES) else 0)
        ));
    }
};
...
pub const IUIFramework = extern struct {
    ...
    pub fn MethodMixin(comptime T: type) type { return struct {
        ...
        // NOTE: method is namespaced with interface name to avoid conflicts for now
        pub fn IUIFramework_LoadUI(self: *const T, instance: ?HINSTANCE, resourceName: ?[*:0]align(1) const u16) callconv(.Inline) HRESULT {
...
pub const UI_EVENTPARAMS_COMMAND = extern struct {
    CommandID: u32,
    CommandName: ?[*:0]align(1) const u16,
    ParentCommandID: u32,
    ParentCommandName: ?[*:0]align(1) const u16,
    SelectionIndex: u32,

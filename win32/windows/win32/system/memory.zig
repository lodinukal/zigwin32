...
pub const SECTION_FLAGS = enum(u32) {
    ...
    pub fn initFlags(o: struct {
        ...
        return @as(SECTION_FLAGS, @enumFromInt(
              (if (o.ALL_ACCESS == 1) @intFromEnum(SECTION_FLAGS.ALL_ACCESS) else 0)
            | (if (o.QUERY == 1) @intFromEnum(SECTION_FLAGS.QUERY) else 0)
            | (if (o.MAP_WRITE == 1) @intFromEnum(SECTION_FLAGS.MAP_WRITE) else 0)
            | (if (o.MAP_READ == 1) @intFromEnum(SECTION_FLAGS.MAP_READ) else 0)
            | (if (o.MAP_EXECUTE == 1) @intFromEnum(SECTION_FLAGS.MAP_EXECUTE) else 0)
            | (if (o.EXTEND_SIZE == 1) @intFromEnum(SECTION_FLAGS.EXTEND_SIZE) else 0)
            | (if (o.MAP_EXECUTE_EXPLICIT == 1) @intFromEnum(SECTION_FLAGS.MAP_EXECUTE_EXPLICIT) else 0)
        ));
    }
};
...
pub const SETPROCESSWORKINGSETSIZEEX_FLAGS = enum(u32) {
    IN_ENABLE = 1,
    IN_DISABLE = 2,
    AX_ENABLE = 4,
    AX_DISABLE = 8,
    ...
    pub fn initFlags(o: struct {
        ...
        return @as(SETPROCESSWORKINGSETSIZEEX_FLAGS, @enumFromInt(
              (if (o.IN_ENABLE == 1) @intFromEnum(SETPROCESSWORKINGSETSIZEEX_FLAGS.IN_ENABLE) else 0)
            | (if (o.IN_DISABLE == 1) @intFromEnum(SETPROCESSWORKINGSETSIZEEX_FLAGS.IN_DISABLE) else 0)
            | (if (o.AX_ENABLE == 1) @intFromEnum(SETPROCESSWORKINGSETSIZEEX_FLAGS.AX_ENABLE) else 0)
            | (if (o.AX_DISABLE == 1) @intFromEnum(SETPROCESSWORKINGSETSIZEEX_FLAGS.AX_DISABLE) else 0)
        ));
    }
};
pub const QUOTA_LIMITS_HARDWS_MIN_ENABLE = SETPROCESSWORKINGSETSIZEEX_FLAGS.IN_ENABLE;
pub const QUOTA_LIMITS_HARDWS_MIN_DISABLE = SETPROCESSWORKINGSETSIZEEX_FLAGS.IN_DISABLE;
pub const QUOTA_LIMITS_HARDWS_MAX_ENABLE = SETPROCESSWORKINGSETSIZEEX_FLAGS.AX_ENABLE;
pub const QUOTA_LIMITS_HARDWS_MAX_DISABLE = SETPROCESSWORKINGSETSIZEEX_FLAGS.AX_DISABLE;
...
) callconv(@import("std").os.windows.WINAPI) ?MEMORY_MAPPED_VIEW_ADDRESS;
...
pub extern "kernel32" fn LocalAlloc(
    ...
    uBytes: usize,
) callconv(@import("std").os.windows.WINAPI) ?HLOCAL;

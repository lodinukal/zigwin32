...
pub const EXIT_WINDOWS_FLAGS = enum(u32) {
    ...
    BOOTOPTIONS = 16777216,
    ARSO = 67108864,
    CHECK_SAFE_FOR_SERVER = 134217728,
    SYSTEM_INITIATED = 268435456,
    _,
    pub fn initFlags(o: struct {
        LOGOFF: u1 = 0,
        SHUTDOWN: u1 = 0,
        ...
        QUICKRESOLVE: u1 = 0,
        RESTARTAPPS: u1 = 0,
        HYBRID_SHUTDOWN: u1 = 0,
        BOOTOPTIONS: u1 = 0,
        ARSO: u1 = 0,
        CHECK_SAFE_FOR_SERVER: u1 = 0,
        SYSTEM_INITIATED: u1 = 0,
    }) EXIT_WINDOWS_FLAGS {
        return @as(EXIT_WINDOWS_FLAGS, @enumFromInt(
              (if (o.LOGOFF == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.LOGOFF) else 0)
            | (if (o.SHUTDOWN == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.SHUTDOWN) else 0)
            | (if (o.REBOOT == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.REBOOT) else 0)
            | (if (o.FORCE == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.FORCE) else 0)
            | (if (o.POWEROFF == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.POWEROFF) else 0)
            | (if (o.FORCEIFHUNG == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.FORCEIFHUNG) else 0)
            | (if (o.QUICKRESOLVE == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.QUICKRESOLVE) else 0)
            | (if (o.RESTARTAPPS == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.RESTARTAPPS) else 0)
            | (if (o.HYBRID_SHUTDOWN == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.HYBRID_SHUTDOWN) else 0)
            | (if (o.BOOTOPTIONS == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.BOOTOPTIONS) else 0)
            | (if (o.ARSO == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.ARSO) else 0)
            | (if (o.CHECK_SAFE_FOR_SERVER == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.CHECK_SAFE_FOR_SERVER) else 0)
            | (if (o.SYSTEM_INITIATED == 1) @intFromEnum(EXIT_WINDOWS_FLAGS.SYSTEM_INITIATED) else 0)
        ));
    }
};
...
pub const EWX_FORCE = EXIT_WINDOWS_FLAGS.FORCE;
pub const EWX_POWEROFF = EXIT_WINDOWS_FLAGS.POWEROFF;
pub const EWX_FORCEIFHUNG = EXIT_WINDOWS_FLAGS.FORCEIFHUNG;
pub const EWX_QUICKRESOLVE = EXIT_WINDOWS_FLAGS.QUICKRESOLVE;
pub const EWX_RESTARTAPPS = EXIT_WINDOWS_FLAGS.RESTARTAPPS;
pub const EWX_HYBRID_SHUTDOWN = EXIT_WINDOWS_FLAGS.HYBRID_SHUTDOWN;
pub const EWX_BOOTOPTIONS = EXIT_WINDOWS_FLAGS.BOOTOPTIONS;
pub const EWX_ARSO = EXIT_WINDOWS_FLAGS.ARSO;
pub const EWX_CHECK_SAFE_FOR_SERVER = EXIT_WINDOWS_FLAGS.CHECK_SAFE_FOR_SERVER;
pub const EWX_SYSTEM_INITIATED = EXIT_WINDOWS_FLAGS.SYSTEM_INITIATED;
...
pub extern "user32" fn ShutdownBlockReasonCreate(
    hWnd: ?HWND,
    pwszReason: ?[*:0]align(1) const u16,
) callconv(@import("std").os.windows.WINAPI) BOOL;

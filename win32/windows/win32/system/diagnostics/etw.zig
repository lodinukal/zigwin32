...
pub const EVENT_TRACE_TYPE_CONFIG_PHYSICALDISK_EX = @as(u32, 19);
...
pub const PETW_BUFFER_CALLBACK = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
...
pub const ETW_PROCESS_TRACE_MODES = enum(i32) {
    NONE = 0,
    RAW_TIMESTAMP = 1,
};
pub const ETW_PROCESS_TRACE_MODE_NONE = ETW_PROCESS_TRACE_MODES.NONE;
pub const ETW_PROCESS_TRACE_MODE_RAW_TIMESTAMP = ETW_PROCESS_TRACE_MODES.RAW_TIMESTAMP;
...
pub const PETW_BUFFER_COMPLETION_CALLBACK = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        Buffer: ?*const ETW_BUFFER_HEADER,
        CallbackContext: ?*anyopaque,
    ) callconv(@import("std").os.windows.WINAPI) void,
...
pub const TracePmcSessionInformation = TRACE_QUERY_INFO_CLASS.TracePmcSessionInformation;
...
pub const EtwQueryLogFileHeader = ETW_PROCESS_HANDLE_INFO_TYPE.LogFileHeader;
...
pub const ITraceEvent = extern struct {
    ...
    pub fn MethodMixin(comptime T: type) type { return struct {
        ...
        // NOTE: method is namespaced with interface name to avoid conflicts for now
        pub fn ITraceEvent_SetTimeStamp(self: *const T, TimeStamp: ?*i64) callconv(.Inline) HRESULT {
...
pub extern "advapi32" fn RegisterTraceGuidsW(
    ...
    MofImagePath: ?[*:0]align(1) const u16,
    MofResourceName: ?[*:0]align(1) const u16,
...
pub extern "advapi32" fn RegisterTraceGuidsA(
    ...
    GuidCount: u32,
    TraceGuidReg: ?[*]TRACE_GUID_REGISTRATION,
    MofImagePath: ?[*:0]align(1) const u8,
    MofResourceName: ?[*:0]align(1) const u8,
    RegistrationHandle: ?*u64,
) callconv(@import("std").os.windows.WINAPI) u32;
...
pub extern "advapi32" fn OpenTraceFromBufferStream(
    Options: ?*const ETW_OPEN_TRACE_OPTIONS,
    BufferCompletionCallback: ?PETW_BUFFER_COMPLETION_CALLBACK,
    BufferCompletionContext: ?*anyopaque,
) callconv(@import("std").os.windows.WINAPI) u64;

pub extern "advapi32" fn OpenTraceFromRealTimeLogger(
    LoggerName: ?[*:0]align(1) const u16,
    Options: ?*const ETW_OPEN_TRACE_OPTIONS,
    LogFileHeader: ?*TRACE_LOGFILE_HEADER,
) callconv(@import("std").os.windows.WINAPI) u64;

pub extern "advapi32" fn OpenTraceFromRealTimeLoggerWithAllocationOptions(
    LoggerName: ?[*:0]align(1) const u16,
    Options: ?*const ETW_OPEN_TRACE_OPTIONS,
...
pub extern "advapi32" fn OpenTraceFromFile(
    LogFileName: ?[*:0]align(1) const u16,
    Options: ?*const ETW_OPEN_TRACE_OPTIONS,
    LogFileHeader: ?*TRACE_LOGFILE_HEADER,
) callconv(@import("std").os.windows.WINAPI) u64;

pub extern "advapi32" fn ProcessTraceBufferIncrementReference(
    TraceHandle: u64,
    Buffer: ?*const ETW_BUFFER_HEADER,
) callconv(@import("std").os.windows.WINAPI) u32;

pub extern "advapi32" fn ProcessTraceBufferDecrementReference(
    Buffer: ?*const ETW_BUFFER_HEADER,
) callconv(@import("std").os.windows.WINAPI) u32;

pub extern "advapi32" fn ProcessTraceAddBufferToBufferStream(
    TraceHandle: u64,
    // TODO: what to do with BytesParamIndex 2?
    Buffer: ?*const ETW_BUFFER_HEADER,
    BufferSize: u32,
...
pub extern "advapi32" fn CveEventWrite(
    CveId: ?[*:0]align(1) const u16,
    AdditionalDetails: ?[*:0]align(1) const u16,
...
test {
    ...
    if (@hasDecl(@This(), "PETW_BUFFER_CALLBACK")) { _ = PETW_BUFFER_CALLBACK; }
    if (@hasDecl(@This(), "PETW_BUFFER_COMPLETION_CALLBACK")) { _ = PETW_BUFFER_COMPLETION_CALLBACK; }

...
pub const HVSOCKET_HIGH_VTL = @as(u32, 8);
...
pub const WHV_X64_CPUID_RESULT2_FLAGS = enum(i32) {
    SubleafSpecific = 1,
    VpSpecific = 2,
    _,
...
pub const WHV_MAP_GPA_RANGE_FLAGS = enum(i32) {
    None = 0,
    Read = 1,
    Write = 2,
    Execute = 4,
...
pub const WHV_TRANSLATE_GVA_FLAGS = enum(i32) {
    None = 0,
    ValidateRead = 1,
    ValidateWrite = 2,
    ValidateExecute = 4,
...
pub const WHV_ALLOCATE_VPCI_RESOURCE_FLAGS = enum(i32) {
    None = 0,
    AllowDirectP2P = 1,
    _,
    pub fn initFlags(o: struct {
...
pub const WHV_CREATE_VPCI_DEVICE_FLAGS = enum(i32) {
    None = 0,
    PhysicallyBacked = 1,
...
pub const WHV_VPCI_MMIO_RANGE_FLAGS = enum(i32) {
    ReadAccess = 1,
    WriteAccess = 2,
    _,
    pub fn initFlags(o: struct {
...
pub const WHV_VPCI_INTERRUPT_TARGET_FLAGS = enum(i32) {
    None = 0,
    Multicast = 1,
    _,
    pub fn initFlags(o: struct {
...
pub const HDV_DEVICE_HOST_FLAGS = enum(i32) {
    None = 0,
    InitializeComSecurity = 1,
    _,
    pub fn initFlags(o: struct {
        None: u1 = 0,
        InitializeComSecurity: u1 = 0,
    }) HDV_DEVICE_HOST_FLAGS {
        return @as(HDV_DEVICE_HOST_FLAGS, @enumFromInt(
              (if (o.None == 1) @intFromEnum(HDV_DEVICE_HOST_FLAGS.None) else 0)
            | (if (o.InitializeComSecurity == 1) @intFromEnum(HDV_DEVICE_HOST_FLAGS.InitializeComSecurity) else 0)
        ));
    }
};
pub const HdvDeviceHostFlagNone = HDV_DEVICE_HOST_FLAGS.None;
pub const HdvDeviceHostFlagInitializeComSecurity = HDV_DEVICE_HOST_FLAGS.InitializeComSecurity;
...
pub const HDV_MMIO_MAPPING_FLAGS = enum(i32) {
    None = 0,
    Writeable = 1,
    Executable = 2,
    _,
...
pub const HDV_PCI_DEVICE_SET_CONFIGURATION = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        ...
        configurationValues: [*]const ?[*:0]align(1) const u16,
    ) callconv(@import("std").os.windows.WINAPI) HRESULT,
    else => *const fn(
        deviceContext: ?*anyopaque,
        configurationValueCount: u32,
...
pub const GUEST_SYMBOLS_PROVIDER_DEBUG_INFO_CALLBACK = switch (@import("builtin").zig_backend) {
    .stage1 => fn(
        InfoMessage: ?[*:0]align(1) const u8,
    ) callconv(@import("std").os.windows.WINAPI) void,
    else => *const fn(
        InfoMessage: ?[*:0]align(1) const u8,
    ) callconv(@import("std").os.windows.WINAPI) void,
...
pub const MODULE_INFO = extern struct {
    ProcessImageName: ?[*:0]align(1) const u8,
    Image: DOS_IMAGE_INFO,
};


//--------------------------------------------------------------------------------
// Section: Functions (126)
//--------------------------------------------------------------------------------
...
pub extern "vmdevicehost" fn HdvInitializeDeviceHostEx(
    computeSystem: HCS_SYSTEM,
...
pub extern "vmsavedstatedumpprovider" fn LocateSavedStateFiles(
    vmName: ?[*:0]align(1) const u16,
    snapshotName: ?[*:0]align(1) const u16,
    binPath: ?*?PWSTR,
    vsvPath: ?*?PWSTR,
    vmrsPath: ?*?PWSTR,
) callconv(@import("std").os.windows.WINAPI) HRESULT;
...
pub extern "vmsavedstatedumpprovider" fn LoadSavedStateFiles(
    binFile: ?[*:0]align(1) const u16,
    vsvFile: ?[*:0]align(1) const u16,
    vmSavedStateDumpHandle: ?*?*anyopaque,
) callconv(@import("std").os.windows.WINAPI) HRESULT;
...
pub extern "vmsavedstatedumpprovider" fn LoadSavedStateSymbolProvider(
    vmSavedStateDumpHandle: ?*anyopaque,
    userSymbols: ?[*:0]align(1) const u16,
    force: BOOL,
) callconv(@import("std").os.windows.WINAPI) HRESULT;
...
const DEVICE_POWER_STATE = @import("../../../windows/win32/system/power.zig").DEVICE_POWER_STATE;
const HANDLE = @import("../../../windows/win32/foundation.zig").HANDLE;
const HCS_SYSTEM = @import("../../../windows/win32/system/host_compute_system.zig").HCS_SYSTEM;
